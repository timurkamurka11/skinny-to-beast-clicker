...
namespace SkinnyToBeast.Gameplay.Patch4
{
    ...
    public sealed class Patch4AnimationRoomReviewDriver : MonoBehaviour
    {
        ...
        private IEnumerator RunReview()
        {
            ...
            for (int i = 0; i < report.clips.Count; i++)
            {
                ...
                report.visualSanityPassed &=
                    ...
                    report.clips[i].singleCompleteFramePassed;
                report.visibleMotionPassed &=
            ...
            int clipIndex)
        {
            ...
            while (elapsed < captureAt)
            {
                ...
                v23FullFramePresentation.SetReviewPose(
                    ...
                    Mathf.Clamp01(
                        elapsed /
                        Mathf.Max(0.001f, reviewDuration)));
                yield return null;
            }
                 ...
                 phaseIndex++)
            {
                ...
                phaseFramesCaptured &=
                    ...
                    string.Equals(
                        ...
                        clipReport.clipName,
                        StringComparison.Ordinal) &&
                    v23FullFramePresentation.ActiveFrameIndex == phaseIndex &&
        ...
        private void RestoreAnimatorAfterClip()
        {
            ...
            if (v23FullFramePresentation != null)
            {
                v23FullFramePresentation.SetReviewActive(false);
            }
        ...
        private static bool RequiresFocusedFaceMotion(string clipName)
        {
            return
                string.Equals(
                    clipName,
            ...
            ClipReview clipReport)
        {
            clipReport.singleCompleteFramePassed =
                ...
                string.Equals(
                    ...
                    clipReport.clipName,
                    StringComparison.Ordinal);
            if (!clipReport.singleCompleteFramePassed)
            {
                report.error = AppendError(
                    report.error,
                    clipReport.clipName +
            ...
            ClipReview clipReport)
        {
            ...
            if (RequiresFocusedFaceMotion(clipReport.clipName))
            {
            ...
            ClipReview clipReport)
        {
            ...
            clipReport.minimumFaceMotionCoverage =
                MinimumFocusedFaceMotionCoverage;
            if (current == null ||
                current.Count != screenWidth * screenHeight ||
                clipStartPixels == null ||
                clipStartPixels.Length != current.Count ||
            ...
            if (!clipReport.focusedFaceMotionPassed)
            {
                report.error = AppendError(
                    report.error,
                    clipReport.clipName +
                    ": painted facial motion is too small inside the face " +
                    "region (" +
            ...
            int screenHeight)
        {
            RectTransform root =
                v23FullFramePresentation != null &&
                v23FullFramePresentation.IsDisplaying &&
                v23FullFramePresentation.PresentationRoot != null
                    ? v23FullFramePresentation.PresentationRoot
